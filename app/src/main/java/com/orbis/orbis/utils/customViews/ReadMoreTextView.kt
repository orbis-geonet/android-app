...
class ReadMoreTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    private var text_value: CharSequence? = null
    ...
    private val displayableText: CharSequence?
        private get() = getTrimmedText(text_value)
    ...
    private fun updateCollapsedText(): CharSequence {
        var trimEndIndex = text_value!!.length
        when (trimMode) {
            TRIM_MODE_LINES -> {
        ...
        Log.d("readMoreCheck", text_value.toString() + " " + trimEndIndex)
        try {
            val s = SpannableStringBuilder(text_value, 0, trimEndIndex)
                .append(ELLIPSIZE)
    ...
    private fun updateExpandedText(): CharSequence? {
        if (showTrimExpandedText) {
            val s = SpannableStringBuilder(text_value, 0, text_value!!.length).append(" ").append(trimExpandedText)
    ...
    private inner class ReadMoreClickableSpan : ClickableSpan() {
        ...
        override fun updateDrawState(ds: TextPaint) {
            ...
            ds.typeface= ResourcesCompat.getFont(context, R.font.roboto_black)
        }
    ...
    init {
        ...
        colorClickableText = typedArray.getColor(
            ...
            ContextCompat.getColor(context, R.color.view_more_blue)
        )
