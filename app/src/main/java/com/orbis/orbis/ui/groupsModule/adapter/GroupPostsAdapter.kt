...
class GroupPostsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            TYPE_TOP    -> ViewHolderTop(inflate(R.layout.item_group_details_top))
    ...
    private fun bindImage(holder: ViewHolderPost, position: Int) {
        ...
        b.userMenuIv.setOnClickListener { menuClick(b.userMenuIv, post, position, "POST") }
    ...
    private fun bindEvent(holder: ViewHolderEvent, position: Int) {
        ...
        if (!post.mediaUrls.isNullOrEmpty()) {
            val storage = Firebase.storage.getReference(
                Constants.EVENT_STORAGE + Utils.getImageUrl680(post.mediaUrls[0])
            )
        ...
        b.userMenuIv2.setOnClickListener { menuClick(b.userMenuIv2, post, position, "POST") }
    ...
    private fun bindSlider(holder: ViewHolderSlider, position: Int) {
        ...
        val mViewPagerAdapter = ViewPagerAdapter(position, context, slider, interaction, null)
        b.checkInViewpager.adapter = mViewPagerAdapter
        b.checkInViewpager.offscreenPageLimit = slider.size
    ...
    private fun bindTop(holder: ViewHolderTop) {
        val b = holder.binding ?: return
        val groupDetails = list[0].group
        b.data = groupDetails
        ViewUtils.loadGroupPhoto(context, b.shadowCircle as ImageView, groupDetails?.imageName)

        b.textView5.text = Constants.placeTopTitle
        ...
        b.groupNameTv.text = groupDetails?.name
        shrinkIfMultiline(b.groupNameTv)
        b.groupDescTv.setContent(groupDetails?.description)
        b.groupDescTv.setTextMaxLength(150)
        b.groupDescTv.setSeeMoreTextColor(R.color.view_more_blue)

        Utils.populateRankIcon(context, groupDetails!!.rankDiffType, b.statusIv)
        b.placesTv.text = groupDetails.placesCount.toString()
        b.membersTv.text = groupDetails.membersCount.toString()

        b.subscriptionActivityBtn.show(groupDetails.isMainAdmin && groupDetails.hasSubscription)
        b.subscriptionButtonsContainer.show(Constants.currentCountryName == "Brazil")
        b.createSubscriptionBtn.text = context.getString(
            if (groupDetails.hasSubscription) R.string.edit_subscription else R.string.create_subscription
        )
        b.subscriptionBtn.text = context.getString(R.string.subscription)

        b.membersTv.setOnClickListener { interaction.onMemberClick() }
        b.membersTitle.setOnClickListener { interaction.onMemberClick() }
        b.placesTv.setOnClickListener { interaction.onPlaceTitleClick() }
        b.placesTitle.setOnClickListener { interaction.onPlaceTitleClick() }
        b.groupMenuIv.setOnClickListener { interaction.onMainMenuClick(b.groupMenuIv) }
        b.createSubscriptionBtn.setOnClickListener { interaction.onCreateSubscriptionClick() }
        b.subscriptionActivityBtn.setOnClickListener { interaction.onSubscriptionActivityClick() }
        b.subscriptionBtn.setOnClickListener { interaction.onSubscriptionClick() }
    }
    ...
    private fun bindAuthorHeader(
        ...
        if (post.group != null) {
            ...
            userNameTv.setOnClickListener { interaction.onItemClicked(0, post) }
            userCircle.setOnClickListener   { interaction.onItemClicked(0, post) }
    ...
    private fun menuClick(anchor: ImageView, post: FeedPost, position: Int, type: String) {
        ...
        // Only show Share and Report to logged-in users
        menu.findItem(R.id.menuItem_share)?.isVisible = isLoggedIn
        menu.findItem(R.id.menuItem_report)?.isVisible = isLoggedIn

        // Delete only for post owner or group admin
        if (post.user != null &&
            (post.user?.userKey == PrefManager(context).getUserKey() || list[0].group?.isAdmin == true)) {
            menu.add("Delete")
        }
        ...
        popup.setOnMenuItemClickListener { menuItemClicked(it, post, position, type) }
        popup.show()
    }

    private fun menuItemClicked(item: MenuItem, post: FeedPost, position: Int, type: String = "POST"): Boolean {
        when (item.title.toString()) {
            "Share"  -> interaction.onShare(post.postKey, type)
            "Delete" -> interaction.onPostDelete(position, post.postKey, position)
            "Report" -> interaction.onReport(post.postKey)
        }
        return true
    }
    ...
    fun setSubscriptionCreatedView(isCreated: Boolean) {
        val item = list[0]
        item.group?.hasSubscription = isCreated
        notifyItemChanged(0, item)
    }

    fun setSubscriptionActivatedView(isActivated: Boolean) {
        val item = list[0]
        item.group?.isSubscriptionActivate = isActivated
        notifyItemChanged(0, item)
    }

    fun setSubscriptionSubscriberView(isSubscriber: Boolean) {
        val item = list[0]
        item.group?.isSubscriber = isSubscriber
        notifyItemChanged(0, item)
    }
    ...
    inner class ViewHolderTop(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val binding: ItemGroupDetailsTopBinding? = DataBindingUtil.bind(itemView)
    }
    ...
    interface PostCardInteraction {
        ...
        fun onShare(postKey: String, type: String)
