...
class VideoPreviewActivity : LassiBaseActivity() {
    ...
    private val logTag = "LassiVideoPreviewActivity"
    ...
    private fun setThemeAttributes() {
        ...
        with(LassiConfig.getConfig()) {
            ...
            val upArrow = ContextCompat.getDrawable(this@VideoPreviewActivity, R.drawable.ic_back_white)
    ...
    private fun onFileScanComplete(uri: Uri?, path: String?) {
        uri?.let { returnUri ->
            ...
            try {
                ...
                val scanPath = cursor.getString(cursor.getColumnIndex(MediaStore.Video.Media.DATA))
                val duration = cursor.getLong(cursor.getColumnIndex(MediaStore.Video.VideoColumns.DURATION))
                val miMedia = MiMedia(id, name, scanPath, duration)
