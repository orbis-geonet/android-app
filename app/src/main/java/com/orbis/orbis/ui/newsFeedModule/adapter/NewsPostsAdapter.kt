...
class NewsPostsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            TYPE_TOP    -> ViewHolderTop(inflate(R.layout.item_news_feed_top))
            TYPE_AD     -> AdViewHolder(LayoutInflater.from(context).inflate(R.layout.item_native_ad, parent, false))
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder.itemViewType) {
            ...
            TYPE_AD     -> bindAd(holder as AdViewHolder)
        }
    }
    ...
    private fun bindTop(holder: ViewHolderTop) {
        val b = holder.binding ?: return
        storyAdapter = StoryAdapter(context, list[0].stories)
        b.statusRv.layoutManager =
            LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false)
        b.statusRv.adapter = storyAdapter
    }

    private fun bindAd(holder: AdViewHolder) {
        val adLoader = AdLoader.Builder(context, Constants.NATIVE_AD_ID)
            .forNativeAd { nativeAd ->
                holder.nativeAdView.setNativeAd(nativeAd)
                try {
                    if (nativeAd.icon != null) {
                        (holder.nativeAdView.iconView as ImageView).setImageDrawable(nativeAd.icon!!.drawable)
                    } else {
                        holder.nativeAdView.iconView!!.visibility = View.GONE
                    }
                    val adImage = if (!nativeAd.images.isNullOrEmpty()) nativeAd.images[0]!!.drawable
                    else nativeAd.icon?.drawable
                    adImage?.let { (holder.nativeAdView.imageView as ImageView).setImageDrawable(it) }

                    if (nativeAd.headline != null) {
                        (holder.nativeAdView.headlineView as TextView).text = nativeAd.headline
                    } else {
                        holder.nativeAdView.headlineView!!.visibility = View.INVISIBLE
                    }
                    if (nativeAd.body != null) {
                        (holder.nativeAdView.bodyView as TextView).text = nativeAd.body
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            ...
            .withAdListener(object : AdListener() {
                override fun onAdFailedToLoad(adError: LoadAdError) {
                    Log.e("nativeAdFailed", adError.message)
                }
            })
    ...
    /**
     * Binds the author/group header that is identical across IMAGE, AUDIO, VIDEO, TEXT view types.
     * Shows the group avatar + user avatar corner badge when the post belongs to a group;
     * shows just the user avatar otherwise.
     */
                ...
                override fun onLoadFailed(e: GlideException?, model: Any?, target: Target<Drawable>?, isFirstResource: Boolean): Boolean {
                    if (retryCount < 5) {
                        Handler(Looper.getMainLooper()).postDelayed(
                            { loadVideoThumb(thumbRef, imageView, ctx, retryCount + 1) },
                            3000
                        )
                    }
                    return true
    ...
    private fun menuItemClicked(item: MenuItem, post: FeedPost, position: Int): Boolean {
        when (item.title.toString()) {
            ...
            context.getString(R.string.report) -> {
                if (PrefManager(context).getIdToken().isNullOrEmpty()) redirectToLogin()
                else interaction.onReport(post.postKey)
            }
        }
        return true
    }
    ...
    fun setupTabs(position: Int, tabLayout: TabLayout, viewPager: ViewPager) {
        ...
        repeat(list[position].slider.size) { tabLayout.addTab(tabLayout.newTab().setIcon(null)) }
    ...
    inner class ViewHolderTop(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val binding: ItemNewsFeedTopBinding? = DataBindingUtil.bind(itemView)
    }

    inner class AdViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val nativeAdView: NativeAdView = (itemView as NativeAdView).also { nav ->
            nav.iconView          = nav.findViewById(R.id.ad_app_icon)
            nav.headlineView      = nav.findViewById(R.id.ad_headline)
            nav.imageView         = nav.findViewById(R.id.ad_image)
            nav.callToActionView  = nav.findViewById(R.id.visit_btn)
        }
    }
}