...
public class CropImageView extends FrameLayout {
    ...
    void onSetImageUriAsyncComplete(BitmapLoadingWorkerTask.Result result) {
        ...
        if (result.error == null) {
            Log.d("CropDebug", "onSetImageUriAsyncComplete - bitmap: " + result.bitmap.getWidth() + " x " + result.bitmap.getHeight());
            Log.d("CropDebug", "onSetImageUriAsyncComplete - uri: " + result.uri);
            Log.d("CropDebug", "onSetImageUriAsyncComplete - loadSampleSize: " + result.loadSampleSize);
            Log.d("CropDebug", "onSetImageUriAsyncComplete - degreesRotated: " + result.degreesRotated);
    ...
    private void setBitmap(
        ...
        if (mBitmap == null || !mBitmap.equals(bitmap)) {
            ...
            post(() -> applyImageMatrix(getWidth(), getHeight(), true, false));
    ...
    private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
        android.util.Log.d("CropDebug", "applyImageMatrix - width: " + width + " height: " + height + " bitmapW: " + (mBitmap != null ? mBitmap.getWidth() : "null") + " bitmapH: " + (mBitmap != null ? mBitmap.getHeight() : "null") + " scaleType: " + mScaleType + " zoom: " + mZoom);
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            float scaleX = width / BitmapUtils.getRectWidth(mImagePoints);
            float scaleY = height / BitmapUtils.getRectHeight(mImagePoints);
