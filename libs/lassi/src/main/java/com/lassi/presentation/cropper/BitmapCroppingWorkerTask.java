...
final class BitmapCroppingWorkerTask
    ...
    public static Bitmap getCircularBitmap(Bitmap square) {
        ...
        // Use the smaller dimension to ensure a true square canvas for the circle
        int size = Math.min(square.getWidth(), square.getHeight());
        int offsetX = (square.getWidth() - size) / 2;
        int offsetY = (square.getHeight() - size) / 2;
        ...
        paint.setColor(0xFFFFFFFF); // must be opaque for SRC_IN to work
        ...
        // Draw only the center square portion of the source bitmap
        Rect src = new Rect(offsetX, offsetY, offsetX + size, offsetY + size);
