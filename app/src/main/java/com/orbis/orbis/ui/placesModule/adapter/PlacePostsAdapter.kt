...
import com.orbis.orbis.extensions.compactFormat
...
class PlacePostsAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            TYPE_SLIDER -> ViewHolderSlider(inflate(R.layout.each_post_slider_place_layout))
            ...
            TYPE_TOP    -> viewHolderTop
                ?: ViewHolderTop(inflate(R.layout.place_page_top)).also { viewHolderTop = it }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder.itemViewType) {
            ...
            TYPE_TOP    -> bindTop(holder as ViewHolderTop)
        }
    }
    ...
    private fun bindEvent(holder: ViewHolderEvent, position: Int) {
        ...
        if (!post.mediaUrls.isNullOrEmpty()) {
            val storage = Firebase.storage.getReference(
                Constants.EVENT_STORAGE + Utils.getImageUrl400(post.mediaUrls[0])
            )
    ...
    private fun bindSlider(holder: ViewHolderSlider, position: Int) {
        ...
        b.checkInViewpager.adapter = PostPagerAdapter(position, context, slider, interaction)
        setupTabs(position, b.tabLayout, b.checkInViewpager)
        b.executePendingBindings()
    }

    private fun bindTop(holder: ViewHolderTop) {
        val b = holder.binding ?: return
        val placeDetails = list[0].place
        b.data = placeDetails
        b.groupImage2.borderWidth = 4
        b.feedTitleTv.text = Constants.placeTopTitle

        applyPlaceFeatures(holder, placeDetails)

        if (placeDetails?.dominantGroup != null) {
            b.groupImage2.setOnClickListener { interaction2.onTitleClick(placeDetails.dominantGroup) }
            b.groupNameTv.setOnClickListener { interaction2.onTitleClick(placeDetails.dominantGroup) }
        }
        b.confirmIv.setOnClickListener { interaction.followPlace() }

        if (list[0].showNoItem == true) {
            b.noItem.visibility = View.VISIBLE
            b.noPhotoTv3.text = list[0].noItemTitle
        } else {
            b.noItem.visibility = View.GONE
        }

        val followedColor = if (placeDetails?.following == true) R.color.followedState else R.color.grey_state
        b.confirmIv.setColorFilter(
            ContextCompat.getColor(context, followedColor),
            android.graphics.PorterDuff.Mode.SRC_IN
        )

        b.eventTv.setOnLongClickListener { interaction.editDescription(); true }

        // Map setup
        val mapFragment = childFragmentManager.findFragmentById(R.id.postMap) as SupportMapFragment
        var mMap: GoogleMap
        val callback = OnMapReadyCallback { googleMap ->
            mMap = googleMap
            mMap.uiSettings.isScrollGesturesEnabled = false
            setupMapOverlay(mMap, googleMap, placeDetails, holder)
        ...
        // Group icons grid
        b.groupImagesRv.layoutManager = GridLayoutManager(context, 3)
        if (!placeDetails?.competingGroups.isNullOrEmpty()) {
            val allGroups = placeDetails!!.competingGroups!!.sortedByDescending { it.percentage }
            val otherGroups = allGroups.filter { it.groupKey != placeDetails.dominantGroupKey }

            b.groupImagesRv.adapter = GroupIconAdapter(interactionGroups, context, ArrayList(otherGroups))

            val progress = allGroups.map { it.percentage }
            val colors   = allGroups.map { Color.parseColor(it.strokeColorHex) }
            b.progressHorizontal.setMaxValue(100f)
            b.progressHorizontal.setProgressValues(ArrayList(progress))
            b.progressHorizontal.setProgressColors(ArrayList(colors))
            b.progressHorizontal.withAnimation(200)
        }

        if (placeDetails?.dominantGroup != null) {
            val storage = Firebase.storage.getReference(
                Constants.GROUP_PHOTO_STORAGE + Utils.getImageUrl200(placeDetails.dominantGroup.imageName!!)
            )
            storage.downloadUrl
                .addOnSuccessListener { Picasso.get().load(it).placeholder(R.drawable.ic_user).into(b.groupImage2) }
                .addOnFailureListener {
                    val fallback = Firebase.storage.getReference(
                        Constants.GROUP_PHOTO_STORAGE + placeDetails.dominantGroup.imageName
                    )
                    fallback.downloadUrl.addOnSuccessListener { uri ->
                        Picasso.get().load(uri).placeholder(R.drawable.ic_user).into(b.groupImage2)
                    }
                }
        }
    }
    ...
    private fun bindAuthorHeader(
        ...
        if (post.group != null) {
            ...
            shrinkIfMultiline(userNameTv)
            userNameTv.setOnClickListener { openProfile(post.user!!) }
            userCircle.setOnClickListener  { openProfile(post.user!!) }
    ...
    private fun bindShareButtons(shareIconIv: View, shareNameTv: View, post: FeedPost) {
        val openUser = View.OnClickListener {
            if (post.user != null) openProfile(post.user!!)
    ...
    // ─── Place top-card helpers ───────────────────────────────────────────────

    private fun applyPlaceFeatures(holder: ViewHolderTop, place: PlaceDetails?) {
        val b = holder.binding ?: return
        b.simpleRatingBar.rating = place?.averageRate ?: 0.0F
        b.rateCountTextView.text = "(${place?.countRates?.compactFormat})"
        b.rateCountTextView.isVisible   = place?.countRates != 0
        b.averageRateTextView.isVisible = place?.averageRate != 0.0F
        b.averageRateTextView.text = String.format("%.2f", place?.averageRate)

        b.ratingContainer.setOnClickListener      { interaction.onRateClicked() }
        b.webSiteImageView.setOnClickListener     { interaction.onWebSiteClicked() }
        b.telephoneImageView.setOnClickListener   { interaction.onPhoneClicked() }
        b.addressImageView.setOnClickListener     { interaction.onAddressClicked() }
        b.openingTimeImageView.setOnClickListener { interaction.onOpeningTimeClicked() }
    }

    private fun setupMapOverlay(
        mMap: GoogleMap,
        googleMap: GoogleMap,
        placeDetails: PlaceDetails?,
        holder: ViewHolderTop
    ) {
        if (placeDetails?.imageName.isNullOrEmpty()) {
            val inflatedView = View.inflate(context, R.layout.custom_place_overlay, null)
            val iconImage    = inflatedView.findViewById<CircleImageView>(R.id.shadow_circle3)
            val placeImage   = inflatedView.findViewById<ImageView>(R.id.placeImage)
            iconImage.borderWidth = 8
            if (placeDetails?.dominantGroup?.strokeColorHex?.isNotEmpty() == true) {
                iconImage.borderColor = Color.parseColor(placeDetails.dominantGroup.strokeColorHex)
            }
            placeImage.setImageDrawable(
                ContextCompat.getDrawable(context, ViewUtils.getPlaceIcon(placeDetails?.type!!)!!)
            )
            val px = (4 * context.resources.displayMetrics.density + 0.5f).toInt()
            placeImage.setPadding(px, px, px, px)

            val iconGenerator = IconGenerator(context).apply {
                setBackground(null)
                setContentView(inflatedView)
            }
            val location = LatLng(placeDetails.coordinates?.latitude!!, placeDetails.coordinates.longitude)
            mMap.addGroundOverlay(
                GroundOverlayOptions()
                    .image(BitmapDescriptorFactory.fromBitmap(iconGenerator.makeIcon()))
                    .anchor(0.5F, 0.5F)
                    .position(location, 400F)
                    .bearing(0f)
                    .clickable(true)
            )
            googleMap.moveCamera(
                CameraUpdateFactory.newLatLngZoom(location, adjustZoomLevelForLatitude(location, 15f))
            )
        } else {
            val storage = Firebase.storage.getReference(
                Constants.PLACE_PICTURES + Utils.getImageUrl200(placeDetails?.imageName!!)
            )
            GlideApp.with(context).load(storage).circleCrop()
                .listener(object : RequestListener<Drawable> {
                    override fun onLoadFailed(e: GlideException?, model: Any?, target: Target<Drawable>?, isFirstResource: Boolean) = false
                    override fun onResourceReady(resource: Drawable?, model: Any?, target: Target<Drawable>?, dataSource: DataSource?, isFirstResource: Boolean): Boolean {
                        activity.runOnUiThread {
                            val inflatedView = View.inflate(context, R.layout.custom_map_places_v2, null)
                            val iconImage    = inflatedView.findViewById<AvatarView>(R.id.ivMapPic)
                            if (!placeDetails.dominantGroup?.strokeColorHex.isNullOrEmpty()) {
                                iconImage.avatarBorderColor = Color.parseColor(placeDetails.dominantGroup!!.strokeColorHex!!)
                            }
                            ...
                            val latLng = LatLng(placeDetails.coordinates!!.latitude, placeDetails.coordinates.longitude)
                            mMap.addGroundOverlay(
                                GroundOverlayOptions()
                                    .image(BitmapDescriptorFactory.fromBitmap(iconGenerator.makeIcon()))
                                    .anchor(0.5F, 0.5F)
                                    .position(latLng, 400F)
                                    .bearing(0f)
                                    .clickable(true)
                            )
                            googleMap.moveCamera(
                                CameraUpdateFactory.newLatLngZoom(latLng, adjustZoomLevelForLatitude(latLng, 15f))
                            )
                        }
                        return true
                    }
        ...
        mMap.setOnGroundOverlayClickListener {
            val popup = PopupMenu(context, holder.binding?.view7, Gravity.CENTER)
            popup.menuInflater.inflate(R.menu.place_overlay_menu, popup.menu)
            popup.setOnMenuItemClickListener { item ->
                when (item?.itemId) {
                    R.id.uploadPlacePhoto -> interaction.uploadPlacePhoto()
                    R.id.getDirections    -> context.startActivity(
                        Intent(Intent.ACTION_VIEW,
                            Uri.parse("http://maps.google.com/maps?q=${placeDetails?.coordinates?.latitude},${placeDetails?.coordinates?.longitude}"))
                    )
                }
                true
            }
    ...
    private fun applyMapStyle(googleMap: GoogleMap) {
        try {
            val success = googleMap.setMapStyle(
                MapStyleOptions.loadRawResourceStyle(context, R.raw.map_style)
            )
            if (!success) Log.e("Style", "Style parsing failed.")
    ...
    fun adjustZoomLevelForLatitude(latLng: LatLng, baseZoomLevel: Float): Float {
        val metersPerPixel = 156543.03392 * cos(Math.toRadians(latLng.latitude))
        return log2(metersPerPixel / baseZoomLevel * 3).toFloat()
    }
    ...
    private fun menuItemClicked(item: MenuItem, post: FeedPost, position: Int): Boolean {
        when (item.title.toString()) {
            context.getString(R.string.share)  -> interaction.onShare(post.postKey)
    ...
    private fun openProfile(user: UserInfo) {
        context.startActivity(
            Intent(context, ProfileActivity::class.java)
                .putExtra("displayName", user.displayName)
                .putExtra("userKey", user.userKey)
        )
    }
    ...
    fun setupTabsImage(position: Int, tabLayout: TabLayout, viewPager: ViewPager) {
        tabLayout.removeAllTabs()
        repeat(list[position].post!!.mediaUrls.size) { tabLayout.addTab(tabLayout.newTab().setIcon(null)) }
        tabLayout.getTabAt(0)?.select()
    ...
    inner class ViewHolderSlider(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val binding: EachPostSliderPlaceLayoutBinding? = DataBindingUtil.bind(itemView)
    }
    ...
    inner class ViewHolderTop(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val binding: PlacePageTopBinding? = DataBindingUtil.bind(itemView)
    }

    // ─── Interaction interface ────────────────────────────────────────────────

    interface PostCardInteraction {
        fun onItemClicked(position: Int, feedPost: FeedPost)
        fun onCommentClicked(position: Int, feedPost: FeedPost)
        fun onLikeClicked(position: Int, feedPost: FeedPost)
        fun onUnlikeClicked(position: Int, feedPost: FeedPost)
        fun onEventAttend(position: Int, feedPost: FeedPost)
        fun onEventClick(position: Int, feedPost: FeedPost)
        fun onEventNotAttend(position: Int, feedPost: FeedPost)
        fun onShare(postKey: String)
        fun onPostDelete(position: Int, postKey: String, rootPos: Int)
        fun onReport(postKey: String)
        fun onPlayerStart(position: Int, player: SimpleExoPlayer)
        fun onMemberClick()
        fun onPlaceTitleClick()
        fun onMainMenuClick(imageView: ImageView)
        fun followPlace()
        fun uploadPlacePhoto()
        fun editDescription()
        fun onRateClicked()
        fun onWebSiteClicked()
        fun onPhoneClicked()
        fun onAddressClicked()
        fun onOpeningTimeClicked()
    }
}