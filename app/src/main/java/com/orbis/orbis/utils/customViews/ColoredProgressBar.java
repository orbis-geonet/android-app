...
public class ColoredProgressBar extends LinearLayout {
    private int id = 1;
    private int radius;
    private int padding;
    ...
    private List<ColoredProgressBar.Progress> progress;

    private int gravity;
    private int textStyle;
    private float textSize;
    private int textAppearance;
    private ArrayList<ColoredProgressBar.Progress> sortProgress;
...
//        setGravity(Gravity.RIGHT);
    ...
    private void addView(Context context) {
        ...
        for (int i = progressLayouts.size(); i < progress.size(); i++) {
            ...
            textView.setId(i);
            int padding = ViewUtils.convertDpToPixel(16F,context);
...
//            linearLayout.addView(textView);
    ...
    public void setProgressValue(float progressValue) {
        ...
        if (progress.size() == 0)
            this.progress.add(
                    new ColoredProgressBar.Progress(progressValue)
            );
        else {
    ...
    private void setProgress(LinearLayout progressLayout, ColoredProgressBar.Progress progress, float previousValue, int i) {
        ...
        TextView textView = (TextView) progressLayout.findViewById(i);
...
//            textView.setTextStyle(textStyle);
    ...
    public void setProgressValues(ArrayList<Float> progressValues) {

        progress.clear();
        Float sum = 0f;
        for (int i = 0; i < progress.size(); i++) {
            progress.get(i).value = progressValues.get(i);
        }
        for (int i = progress.size(); i < progressValues.size(); i++) {
            sum = sum + progressValues.get(i);
            progress.add(new ColoredProgressBar.Progress(sum));
        }
    ...
    public void addProgressValue(float progressValue) {
        progress.add(new ColoredProgressBar.Progress(progressValue));
        sort();
        refresh();
    }

    public void addProgressValue(float progressValue, int color) {
        progress.add(new ColoredProgressBar.Progress(progressValue, color));
        sort();
        refresh();
    }
    ...
    public void setProgressColor(int progressColor) {
        if (progress.size() == 0)
            this.progress.add(
                    new ColoredProgressBar.Progress(progressColor)
            );
        else {
    ...
    public void setProgressColors(ArrayList<Integer> colors) {
        for (int i = 0; i < progress.size(); i++) {
            progress.get(i).color = colors.get(i);
        }
        for (int i = progress.size(); i < colors.size(); i++) {
            progress.add(new ColoredProgressBar.Progress(colors.get(i)));
        }
    ...
    public void setProgressText(String text) {
        if (progress.size() == 0)
            this.progress.add(
                    new ColoredProgressBar.Progress(text)
            );
        else {
    ...
    public void setProgressTexts(int... resIds) {
        ...
        for (int i = progress.size(); i < resIds.length; i++) {
            progress.add(new ColoredProgressBar.Progress(getResources().getString(resIds[i])));
        }
    ...
    public void setProgressIcon(Drawable drawable) {
        if (progress.size() == 0)
            this.progress.add(
                    new ColoredProgressBar.Progress(drawable)
            );
        else {
    ...
    public void setProgressIcons(Drawable... drawables) {
        ...
        for (int i = progress.size(); i < drawables.length; i++) {
            progress.add(new ColoredProgressBar.Progress(drawables[i]));
        }
    ...
    public void setProgressIcons(int... resIds) {
        ...
        for (int i = progress.size(); i < resIds.length; i++) {
            progress.add(new ColoredProgressBar.Progress(getResources().getDrawable(resIds[i])));
        }
    ...
    private void sort(List<ColoredProgressBar.Progress> progress) {
        Collections.sort(progress, new Comparator<ColoredProgressBar.Progress>() {
            @Override
            public int compare(ColoredProgressBar.Progress lhs, ColoredProgressBar.Progress rhs) {
