...
class DetailsPostsAdapter(
    ...
    private val commentInteraction: CommentsAdapter.CommentInteraction
    ...
    companion object {
        ...
        private const val TYPE_COMMENTS = 6
    }

    // ─── Adapter overrides ────────────────────────────────────────────────────

    override fun getItemViewType(position: Int): Int {
        // Last item in a multi-item list is the comments section
        if (list.size > 1 && position == list.size - 1) return TYPE_COMMENTS
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            TYPE_COMMENTS -> ViewHolderComments(inflate(R.layout.item_comments))
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder.itemViewType) {
            ...
            TYPE_COMMENTS -> bindComments(holder as ViewHolderComments, position)
        }
    }
    ...
    private fun bindImage(holder: ViewHolderPost, position: Int) {
        ...
        try { b.dateTv.text = ViewUtils.convertTimeStampToFormatted(post.timestamp) } catch (e: Exception) { e.printStackTrace() }
    ...
    private fun bindComments(holder: ViewHolderComments, position: Int) {
        val b = holder.binding ?: return
        b.commentsRv.layoutManager = LinearLayoutManager(context)
        b.commentsRv.adapter = CommentsAdapter(context, list[position].comments, commentInteraction)
    }
    ...
    inner class ViewHolderComments(itemView: View) : RecyclerView.ViewHolder(itemView) {
