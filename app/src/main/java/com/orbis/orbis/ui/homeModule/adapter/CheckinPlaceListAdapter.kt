...
class CheckinPlaceListAdapter(
    private val interaction: PlaceCardInteraction,
    private val context: Context,
    private val places: ArrayList<PlaceDetails>
) :
    RecyclerView.Adapter<CheckinPlaceListAdapter.ViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(
            LayoutInflater.from(context)
                .inflate(R.layout.each_checkin_place_item, parent, false)
        )
    }
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val binding: EachCheckinPlaceItemBinding? = DataBindingUtil.bind(itemView)
    }
